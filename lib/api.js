...
async function request(endpoint, options = {}) {
    ...
    if (text) {
        try {
            ...
            // Backend may return HTML (e.g. 404, 502) or plain text
            if (!res.ok) {
                const isHtml = text.trimStart().toLowerCase().startsWith("<!") || text.includes("<html");
                const message = isHtml
                    ? `${res.status} ${res.statusText}. The requested resource may not exist or the server returned an error page.`
                    : `${res.status} ${res.statusText}: ${text.slice(0, 100)}`;
...
function getUploadHeaders() {
    const token = getToken();
    if (!token) {
        const err = new Error("Please log in to upload.");
        err.status = 401;
        throw err;
    }
    // Use Headers so Authorization is always sent (some environments drop it from plain objects)
...
/**
 * upload3DModel — sends 3D files (GLB, GLTF, OBJ, etc.) directly to the backend
 * at BASE_URL/upload with field "image" so the same upload endpoint accepts it.
 * Request goes to the API base URL (not localhost) with the auth token.
 */
...
// Categories — /api/categories (hustle routes: GET / POST / DELETE /:id). Body: name, description?, image?, slug? (flat categories; parentId not set via API)
...
export const updateCategory = async (id, body) => {
    ...
    for (const [path, method] of endpoints) {
        try {
            return await request(path.replace("${id}", id), { method, body: payload });
        } catch (e) {
            lastErr = e;
            if (e?.status !== 404) throw e;
        }
    }
    const err = new Error("Category update not available. Add PUT or PATCH for /categories/:id (or /admin/categories/:id) on your backend.");
    err.status = 404;
    throw err;
};
...
// Brands — /api/brands (GET, POST, PUT /:id, DELETE /:id). Body: name, description?, image?
...
export const updateBrand = (id, body) =>
    request(`/brands/${id}`, { method: "PUT", body: JSON.stringify(body) });
...
// Admin: product reviews — show / hide on storefront (GET list, PATCH visibility)
export const getAdminReviews = (params = {}) => {
    const q = new URLSearchParams();
    if (params.page != null) q.set("page", String(params.page));
    if (params.limit != null) q.set("limit", String(params.limit));
    if (params.productId != null && params.productId !== "") q.set("productId", String(params.productId));
    if (params.search != null && params.search !== "") q.set("search", String(params.search).trim());
    if (params.rating != null && params.rating !== "" && params.rating !== "all") {
        q.set("rating", String(params.rating));
    }
    const query = q.toString();
    return request(`/admin/reviews${query ? "?" + query : ""}`);
};
export const patchReviewsVisibility = (updates) =>
    request("/admin/reviews/visibility", {
        method: "PATCH",
...
export const getOrder = (id) => request(`/admin/orders/${id}`);
...
export const trackAdminOrder = (id) => request(`/admin/orders/${id}/track`);
...
export const updateFaq = (id, body) =>
    request(`/admin/faq/${id}`, { method: "PUT", body: JSON.stringify(body) });
...
// Store Settings
export const getStoreSettings = () => request("/admin/settings");
export const updateStoreSettings = (body) =>
    request("/admin/settings", { method: "PUT", body: JSON.stringify(body) });

/** Cancel order (admin/staff with orders permission). */
export const adminCancelOrder = (id, body = {}) =>
    request(`/admin/orders/${id}/cancel`, { method: "POST", body: JSON.stringify(body) });

/** Team — super-admin only (backend enforces). */
export const listStaffUsers = () => request("/admin/team/staff");
export const createStaffUser = (body) =>
    request("/admin/team/staff", { method: "POST", body: JSON.stringify(body) });
export const updateStaffUser = (id, body) =>
    request(`/admin/team/staff/${id}`, { method: "PUT", body: JSON.stringify(body) });
export const deleteStaffUser = (id) =>
    request(`/admin/team/staff/${id}`, { method: "DELETE" });
...
export const updateCoupon = (id, body) =>
    request(`/coupons/${id}`, { method: "PUT", body: JSON.stringify(body) });
