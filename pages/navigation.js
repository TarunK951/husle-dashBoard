...
export default function NavigationPage() {
  ...
  const updateItemDropdown = (i, field, value) =>
    setItems((s) =>
      s.map((x, j) =>
        j === i
          ? { ...x, dropdown: { ...(x.dropdown || {}), [field]: value } }
          : x,
      ),
    );
        ...
        j === navIndex
          ? {
              ...
              dropdown: {
                ...
                items: [
                  ...(x.dropdown?.items || []),
                  { name: "", description: "", href: "", image: "", icon: "" },
                ],
              },
            }
          : x,
      ...
      <Layout>
        <div className="space-y-6 fade-in max-w-3xl">
          <p className="text-sm text-[#6e6e73]">
            Main nav links (Products, Bundle, Accessories, Support) and
            dropdowns. Order here = order in the navbar.
          </p>
          {error && (
          ...
          {!error && (
            <form onSubmit={handleSave} className="space-y-4">
              {items.map((item, i) => (
                <div
                  key={i}
                  className="border border-black/10 rounded-xl p-4 bg-white space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <label className="flex items-center gap-1.5 whitespace-nowrap text-sm">
                      <input
                        ...
                        onChange={(e) =>
                          updateItem(i, "hasDropdown", e.target.checked)
                        }
                        className="rounded"
                      />
                      Dropdown
                    ...
                    <div className="pl-6 border-l-2 border-black/10 space-y-3">
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            updateItemDropdown(i, "title", e.target.value)
                          }
                          placeholder="Dropdown title"
                        />
                        <select
                          className={INPUT}
                          value={item.dropdown?.layout ?? "row"}
                          onChange={(e) =>
                            updateItemDropdown(i, "layout", e.target.value)
                          }>
                      ...
                      <div className="space-y-2">
                        ...
                        {(item.dropdown?.items || []).map((di, diIdx) => (
                          <div
                            key={diIdx}
                            className="flex flex-wrap gap-2 items-start p-2 rounded-lg bg-[#f5f5f7]">
                            <input
                        ...
                        <button
                          ...
                          className="text-sm text-[#1d1d1f] font-medium flex items-center gap-1">
              ...
              <button
                type="button"
                onClick={addItem}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-dashed border-black/20 text-[#6e6e73] hover:bg-black/5 text-sm">
              ...
              <div className="pt-4">
                <button
                  type="submit"
                  disabled={saving}
                  className="bg-[#1d1d1f] text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-black disabled:opacity-60">
