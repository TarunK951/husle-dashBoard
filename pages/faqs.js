            ...
            {open && (
                <div className="px-5 pb-4 border-t border-black/5">
                    ...
                    <div className="mt-3 flex items-center gap-3">
                        <button
                            onClick={() => onEdit(faq)}
                            className="flex items-center gap-1.5 text-xs font-medium text-[#1d1d1f] hover:underline transition-colors"
                        >
                            Edit
                        </button>
                        <button
            ...
            <Layout>
                <div className="space-y-4 fade-in">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => { setEditId(null); setForm({ question: "", answer: "", category: "general" }); setShow(true); }} className="flex items-center gap-2 bg-[#1d1d1f] text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-black transition-colors">
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-2">
                            {faqs.map((f) => <FaqItem key={f.id} faq={f} onEdit={openEdit} onDelete={handleDelete} />)}
                        </div>
                    ) : null}
                </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg fade-in">
                            <div className="flex items-center justify-between px-6 py-4 border-b border-black/5">
                                <h2 className="font-bold text-[#1d1d1f]">{editId ? "Edit FAQ" : "Add FAQ"}</h2>
                                <button type="button" onClick={() => { setShow(false); setEditId(null); }} className="p-2 rounded-xl hover:bg-black/5"><X size={18} /></button>
                            </div>
                            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                                ...
                                <div className="flex gap-3 pt-1">
                                    <button type="button" onClick={() => { setShow(false); setEditId(null); }} className="flex-1 py-2.5 rounded-xl border border-black/10 text-sm font-medium hover:bg-[#f5f5f7] transition-colors">Cancel</button>
